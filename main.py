...
CHANNEL_ID = "@qawcaze"
ADMIN_ID = 6895966276  # ID-и худро инҷо гузор
...
def start(message):
    ...
    if check_sub(user_id):
        ...
        markup.add(
            types.InlineKeyboardButton(text="𝒅𝒊𝒂𝒎𝒐𝒏𝒅 𝒕𝒐 𝙵𝚛𝚎е 𝙵𝚒𝚛е 💎", callback_data="buy_diamonds"),
...
Ин бот барои алмос ( алмаз ё ки 𝒅𝒊𝒂𝒎𝒐𝒏𝒅 ) гузаронидан ба бозии 𝙵𝚛𝚎е 𝙵𝚒𝚛е аст ‼️
...
Барои харидани алмос лутфан тугмаҳоро интихоб кунед :"""
        ...
        markup.add(
            types.InlineKeyboardButton(text="qawcaz", url="https://t.me/qawcaze"),
...
def ask_id(call):
    msg = bot.send_message(call.message.chat.id, 
                           "Шумо дар ( 𝒅𝒊𝒂𝒎𝒐𝒏𝒅 𝒕𝒐 𝙵𝚛ее 𝙵𝚒𝚛е 💎 ) қарор доред ‼️\n\n"
...
def process_diamond_selection(call):
    ...
    markup = types.InlineKeyboardMarkup(row_width=1)
    markup.add(
        types.InlineKeyboardButton(text="𝙳𝚞𝚜𝚑𝚊н𝚋𝚎 𝙲𝚒𝚝𝚢 💳", callback_data="pay_dc"),
...
@bot.callback_query_handler(func=lambda call: call.data == "pay_dc")
def pay_dc(call):
    chat_id = call.message.chat.id
    ...
    markup = types.InlineKeyboardMarkup(row_width=1)
    markup.add(
        types.InlineKeyboardButton(text="𝙳𝚞𝚜хан𝚋𝚎 𝙲𝚒𝚝𝚢 💳", url=link),
        types.InlineKeyboardButton(text="𝑚𝑜𝑛𝑒𝑦 𝑖𝑠 𝑡𝑜 𝑎𝑑𝑚𝑖𝑛 𝑐𝑜𝑟𝑑 🧑‍💻", callback_data="money_sent_request"),
        types.InlineKeyboardButton(text="БА ҚАФО 🔃", callback_data="buy_diamonds")
    )
    
    text = "Шумо 𝙳𝚞𝚜𝚑𝚊н𝚋𝚎 𝙲𝚒𝚝𝚢 ро интихоб кардед барои супоридани маблағ лутфан ба тугмаи 𝙳𝚞𝚜𝚑𝚊н𝚋𝚎 𝙲𝚒𝚝𝚢 💳 пахш кунед мустақим ба барномаи корт меравед ва оплатит кунед ✅\n\nБаъд аз он ки пулро супоридед тугмаи 𝑚𝑜𝑛𝑒𝑦 𝑖𝑠 𝑡𝑜 𝑎𝑑𝑚𝑖𝑛 𝑐𝑜𝑟𝐝 🧑‍💻 ро пахш кунед :"
    bot.edit_message_text(text, chat_id, call.message.message_id, reply_markup=markup)

@bot.callback_query_handler(func=lambda call: call.data == "pay_not_card")
def pay_not_card(call):
    chat_id = call.message.chat.id
    price = user_data[chat_id]['price']
...
@bot.callback_query_handler(func=lambda call: call.data == "money_sent_request")
def money_sent_request(call):
...
def process_receipt(message):
    ...
    if message.content_type == 'photo':
        ...
        # ХАБАР БА АДМИН
        g_id = user_data.get(chat_id, {}).get('game_id', "?")
        g_pack = user_data.get(chat_id, {}).get('pack_name', "?")
        g_price = user_data.get(chat_id, {}).get('price', "?")
        
        admin_text = f"🔔 ФАРМОИШИ НАВ!\n\n🆔: {g_id}\n🛍️: {g_pack}\n💸: {g_price} 🇹🇯\n👤: @{message.from_user.username}"
